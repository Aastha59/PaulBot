...
# from pymongo import MongoClient
# # from utils.email_utils import send_email, format_conversation_for_email  # ✅ CORRECT
...
# #         print(f"[ERROR] Unexpected format in conversation_id={conversation_id}: {messages}")
# #         return []

# #     try:
# #         return [{"role": m["role"], "content": m["content"]} for m in messages]
# #     except Exception as e:
# #         print(f"[ERROR] Failed to parse messages: {e}, messages={messages}")
# #         return []
...
#         # ✅ Email trigger based on final message
...
# async def send_summary(conversation_id: str = Query(...)):
#     try:
#         messages = list(collection.find(
#             {"conversation_id": conversation_id},
#             sort=[("timestamp", 1)]
...
#         send_email(
#             subject=f"PaulBot Conversation Summary - ID: {conversation_id}",
#             html_content=email_html,
#             recipient="aasthaarya2004@gmail.com"
#         )
...
# from email.mime.multipart import MIMEMultipart
# from email.mime.text import MIMEText
# from utils.email_utils import format_conversation_for_email, send_email_if_triggered
# # from utils.email_utils import send_email, format_conversation_for_email
# # from utils.email_utils import send_email_if_triggered
# # send_email_if_triggered(messages, input.conversation_id)
# app = FastAPI()
...
#         # if reply.lower().strip() in ["Thank you! We will contact you shortly.", "If you don't see what you're looking for, click on your preferred choice and explore more options on our website: PML Holidays Website. Our team will connect with you shortly to curate the perfect trip for you!", "thank you"]:
#         # if input.message.lower().strip() in ["done", "end", "thank you"]:
#         # if any(phrase in reply for phrase in  [
#         #     "Thank you! We will contact you shortly.",
#         #     "If you don't see what you're looking for, click on your preferred choice and explore more options on our website: PML Holidays Website. Our team will connect with you shortly to curate the perfect trip for you!"
#         # ]:
#         # if reply.strip() in trigger_phrases:
#         # if any(trigger_phrase in reply for trigger_phrase in trigger_phrases):
#         # if any(trigger_phrase.lower() in reply.lower() for trigger_phrase in trigger_phrases):
#         #     print("📧 Match found — triggering email!")
#         #     messages = list(collection.find(
#         #         {"conversation_id": input.conversation_id},
#         #         sort=[("timestamp", 1)]
#         #     ))
...
# # ✅ FIXED send-email-summary to accept JSON
# class EmailSummaryRequest(BaseModel):
#     conversation_id: str
#     recipient: str = "aasthaarya2004@gmail.com"
...
# from email.mime.multipart import MIMEMultipart
# from email.mime.text import MIMEText
# # from utils.email_utils import send_email, format_conversation_for_email, should_trigger_email_from_bot
# # from utils.email_utils import send_email, format_conversation_for_email, should_trigger_email_from_bot
# from utils.email_utils import send_email_if_triggered, format_conversation_for_email, should_trigger_email_from_bot
...
#         # ✅ Send email only if PaulBot's reply contains one of the target phrases
#         if should_trigger_email_from_bot(reply):
#             messages = list(collection.find(
#                 {"conversation_id": input.conversation_id},
#                 sort=[("timestamp", 1)]
#             ))
